...
public class RegisterExpenseUseCaseTest
{
    ...
    public async Task Error_Title_Empty()
    {
        ...
        result.Where(exception => exception.GetErrors().Count().Equals(1)
                     && exception.GetErrors().Contains(ResourceErrorMessages.TITLE_REQUIRED));
    }
    ...
    private RegisterExpenseUseCase CreateUseCase(User user)
    {
        ...
        return new RegisterExpenseUseCase(repository, unityOfWork, mapper, loggedUser);
    }
}
